...
private:
    void initializePayloadSDK() {
        try {
            RCLCPP_INFO(this->get_logger(), "Starting PayloadSDK initialization...");
            
            my_payload_ = new PayloadSdkInterface(s_conn);
            RCLCPP_INFO(this->get_logger(), "PayloadSDK interface created");
            
            my_payload_->sdkInitConnection();
            RCLCPP_INFO(this->get_logger(), "SDK connection initialized");
            
            std::this_thread::sleep_for(std::chrono::seconds(2));
            my_payload_->checkPayloadConnection();
            RCLCPP_INFO(this->get_logger(), "Payload connection checked");
            
            try {
                // 只设置视频模式
                RCLCPP_INFO(this->get_logger(), "Setting to video mode...");
                my_payload_->setPayloadCameraMode(CAMERA_MODE_VIDEO);
                std::this_thread::sleep_for(std::chrono::seconds(3));
                
            } catch (const std::exception& e) {
                RCLCPP_ERROR(this->get_logger(), "Failed to set video mode: %s", e.what());
                throw;
            }
            
            RCLCPP_INFO(this->get_logger(), "Camera initialization completed");
            
        } catch (const std::exception& e) {
            RCLCPP_ERROR(this->get_logger(), "PayloadSDK initialization failed: %s", e.what());
            if (my_payload_) {
                try {
                    my_payload_->sdkQuit();
                } catch (...) {}
