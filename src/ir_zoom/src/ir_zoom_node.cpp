...
class IrZoomNode : public rclcpp::Node {
private:
    PayloadSdkInterface* my_payload;
    rclcpp::Subscription<std_msgs::msg::Float32>::SharedPtr zoom_range_sub_;

public:
    IrZoomNode() : Node("ir_zoom_node") {
        RCLCPP_INFO(this->get_logger(), "启动 IR 变焦节点...");
        ...
        // 设置为红外视图
        RCLCPP_INFO(this->get_logger(), "设置为红外视图");
        my_payload->setPayloadCameraParam(PAYLOAD_CAMERA_VIEW_SRC, PAYLOAD_CAMERA_VIEW_IR, PARAM_TYPE_UINT32);
        std::this_thread::sleep_for(std::chrono::seconds(1));

        // 设置初始变焦级别
        #if defined GHADRON
        my_payload->setPayloadCameraParam(PAYLOAD_CAMERA_VIDEO_ZOOM_FACTOR, ZOOM_IR_1X, PARAM_TYPE_UINT32);
        ...
        zoom_range_sub_ = this->create_subscription<std_msgs::msg::Float32>(
            "ir_zoom/range", 10,
            std::bind(&IrZoomNode::zoomRangeCallback, this, std::placeholders::_1));

        RCLCPP_INFO(this->get_logger(), "IR 变焦节点已准备就绪");
    }
...
int main(int argc, char** argv) {
    ...
    rclcpp::spin(std::make_shared<IrZoomNode>());
    rclcpp::shutdown();
    return 0;
}